...
describe("UserService - createUser", () => {
  ...
  it("deve criar usuário com sucesso", async () => {
    ...
    const user = await userService.createUser({ name: "Ryan", email: "r@test.com", password: "12345678" });
    expect(user).toHaveProperty("id");
    expect(user.name).toBe("Ryan");
  });

  it("deve lançar InvalidNameError para nome inválido", async () => {
    await expect(userService.createUser({ name: "R", email: "r@test.com", password: "12345678" }))
      .rejects.toThrow(InvalidNameError);
  });

  it("deve lançar InvalidEmailError para email inválido", async () => {
    await expect(userService.createUser({ name: "Ryan", email: "email-invalido", password: "12345678" }))
  ...
  it("deve lançar EmailAlreadyExistsError se email já existe", async () => {
    ...
    await expect(userService.createUser({ name: "Ryan", email: "r@test.com", password: "12345678" }))
      .rejects.toThrow(EmailAlreadyExistsError);
  });

  it("deve lançar InvalidPasswordError se senha inválida", async () => {
    await expect(userService.createUser({ name: "Ryan", email: "r@test.com", password: "123" }))
